...
(function () { // Code isolation
    ...
    var MIN_CURSOR_UPDATES_INTERVAL_MS = instr.server_config.MAX_EMIT_COUNT_PERIOD / instr.server_config.MAX_EMIT_COUNT * 2;
    ...
    instr.register(cursorTool);
    instr.addToolListeners(cursorTool);
    ...
    function updateMarker() {
        if (!instr.showMarker || !instr.showMyCursor) return;
        var cur_time = Date.now();
        if (cur_time - lastCursorUpdate > MIN_CURSOR_UPDATES_INTERVAL_MS &&
            (sending || instr.curTool.showMarker)) {
            instr.drawAndSend(message, cursorTool);
    ...
    var cursorsElem = instr.svg.getElementById("cursors");
    ...
    function draw(message) {
        ...
        if (instr.isIE) cursor.setAttributeNS(null, "transform", "translate(" + message.x + " " + message.y + ")");
